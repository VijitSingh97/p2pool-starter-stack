...
compose_layout_probe() { # <installed|source>
    local layout="$1" dir="$ONP/$1"
...
assert_eq "installed compose up disables absent source builds (#1967)" \
    "$(compose_layout_probe installed)" "compose up --no-build --pull missing -d"
assert_eq "source compose up preserves development builds (#1967)" \
    "$(compose_layout_probe source)" "compose up --pull never -d"
