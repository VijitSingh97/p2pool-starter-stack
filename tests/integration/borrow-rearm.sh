...
wait_borrow_rearm() {
    ...
    [ -n "${IT_BORROW_REARM_ACK:-}" ] || {
        it_fail "borrowed-pool re-arm handshake configured" "IT_BORROW_REARM_ACK is empty"
        return 1
    }
    ...
    if wait_for 120 2 "the e2e controller to re-arm the borrowed miner pool" test -f "$IT_BORROW_REARM_ACK"; then
