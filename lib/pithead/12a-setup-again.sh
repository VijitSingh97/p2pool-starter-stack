...
# enforced before stdin is read, and that inode replaces the card whole. Owned by the page's user;
# the ack that gates the erase is unchanged.
write_handoff_card() { # <spool-dir>; the card's JSON on stdin
    local tmp
    tmp=$(mktemp "$1/.handoff.json.XXXXXX") || return 1
    if [ -L "$tmp" ] || [ ! -f "$tmp" ] || ! chmod 600 "$tmp"; then
        rm -f "$tmp"
        return 1
    fi
    if ! cat >"$tmp" || ! chown 1000:1000 "$tmp" 2>/dev/null ||
