...
#
# Config and .env carry credentials, unlike optional backup items (#1059/#1244). Called before
# the disk-space check and stack stop, so refusal leaves the box exactly as it was, the same
# promise the space check makes. `-f` also refuses a dangling symlink; the function is directly
# testable without driving the whole backup flow.
