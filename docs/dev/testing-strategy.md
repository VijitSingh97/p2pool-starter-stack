...
| RC2 hostname identity (#1957/#1966): the wizard names the appliance `fixture-box`; kernel hostname, rendered `HOST_IP`, `/api/state`'s header source, served certificate DNS+LAN-IP SANs and mDNS resolution through active Avahi must agree initially, after an unaided reboot and after the A/B migration update. A day-two `fixture-next` preview must require the combined approval path while leaving all readings unchanged | `tests/os/appliance-hostname-leg.sh` + battery `--phase provision` | 1 ✅ (pure verdict controls) · 4 (added — unverified until #1957 and #1959 are integrated into an image and the battery runs) |
| RC2 remaining approval/node rows (#1959/#1898/#1943/#1946): an allow-listed second identity approves an exact payout/auth/control preview while wrong/missing approval and physical-presence-only settings stay refused; a reserved reachable node endpoint passes the host preflight and p2pool's current-startup `uses chain_id` consumer probe | operator-assisted battery row plus the existing `tests/integration/mergemine-probe.sh` verdict | 4 ⏸ (the config approval contract is still changing; no positive result may be recorded until the frozen interface and reserved nodes are exercised) |
